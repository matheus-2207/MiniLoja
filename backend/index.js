    ...
    db.serialize(() => {
      ...
      db.get("SELECT COUNT(*) AS count FROM products", (err, row) => {
        if (row && row.count === 0) {
          ...
          stmt.run('Whey Protein Concentrado 1KG', 'Whey', 109.90, 'Suplemento proteico para ganho de massa muscular.', 'https://via.placeholder.com/300?text=Whey+Protein');
          stmt.run('Creatina Monohidratada 300g', 'Creatina', 89.90, 'Auxilia no aumento do desempenho físico.', 'https://via.placeholder.com/300?text=Creatina');
          stmt.run('Pré Treino Insane', 'Pre Treino', 149.90, 'Energia extrema para seus treinos.', 'https://via.placeholder.com/300?text=Pre+Treino');
          stmt.finalize();
...
app.post('/api/products', authenticate, isAdmin, (req, res) => {
  ...
  db.run("INSERT INTO products (name, category, price, description, image_url) VALUES (?, ?, ?, ?, ?)", 
    [name, category, price, description, image_url || 'https://via.placeholder.com/300'], function(err) {
    if (err) return res.status(500).json({ error: err.message });
...
app.put('/api/products/:id', authenticate, isAdmin, (req, res) => {
  ...
  db.run("UPDATE products SET name = ?, category = ?, price = ?, description = ?, image_url = ? WHERE id = ?", 
    [name, category, price, description, image_url || 'https://via.placeholder.com/300', id], function(err) {
    if (err) return res.status(500).json({ error: err.message });
